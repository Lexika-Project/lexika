...
def get_array_tess(pdf_img, test_bold=False, bold_aprox=0.0013):
    mat = fitz.Matrix(6, 6)
    ...
    if test_bold:
        kernel = np.ones((5, 5), np.floating)
    ...
    for i in range(len(dict_ocr["text"])):
        if dict_ocr["text"][i] != "":
            ...
            special_letters = ["aa", "ää", "ââ", "ee", "éé", "ëë", "èè", "êê", "ii", "îî", "oo", "öö", "ôô", 
                           "uu", "üü", "ùù", "ûû","bw", "gw", "kw", "mw", "pw", "xw","ng", "ny","ü","ä","ö","ë","î","ô","û"]
    ...
    with open(file_input + ".pdf", "rb") as pdf_file:
        ...
        match args.process:
            ...
            case 1:
                ...
                START = 27
                END = 135
        with open(file_output + ".csv", "w", encoding="cp1252") as file:
