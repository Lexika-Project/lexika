...
function create_result(table_database) {
	const resultZone = document.querySelector("#resultZone");
	let current = -1;
	let container, ref, table, title, value;
	for (let elem of table_database) {
		if (current !== elem[2]) {
			current = elem[2];
			...
			title = document.createElement("thead");
			value = document.createElement("tbody");
			table.appendChild(title);
			table.appendChild(value);
			container.appendChild(table);
		}
		if (elem[0] === langueBase) {
			ref.innerText = elem[1];
		} else {
